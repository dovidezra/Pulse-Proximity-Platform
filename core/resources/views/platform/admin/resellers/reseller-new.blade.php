...
$account_owner = Former::select('user_id')
  ->addOption('&nbsp;')
  ->class('select2-required form-control')
  ->name('user_id')
  ->fromQuery($users, 'email', 'id')
  ->required(true)
  ->label(trans('global.account_owner'));
...
?>
      ...
      <div class="col-md-6">
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title">{{ trans('global.mail_settings') }}</h3>
          </div>
          <fieldset class="panel-body">
            ...
            <div class="form-group">
              <?php
                  echo Former::select('mail_driver')
                    ->class('select2-required form-control')
                    ->name('mail_driver')
                    ->options(['' => '&nbsp;', 'smtp' => 'SMTP', 'mailgun' => 'Mailgun'])
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title">Avangate</h3>
          </div>
          <fieldset class="panel-body">

            <div class="form-group">
              <label for="avangate_key">{{ trans('global.key') }}</label>
              <input type="text" class="form-control" name="avangate_key" id="avangate_key" value="" autocomplete="off">
            </div>
